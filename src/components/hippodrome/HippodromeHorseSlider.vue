<template>
    ...
    <NFlex vertical style="width: 100%;">
        <NText italic style="margin-bottom: 10px;">{{ currentHorse?.name ?? "Not Generated" }}</NText>
        <NSlider data-test='horse-slider' class="horse-slider-wrapper" v-model:value="currentHorsePosition"
            :max="currentRound?.distance" :step="0.1" :style="{
                display: 'flex',
                alignItems: 'center',
            ...
            <template #thumb>
                <NIcon-wrapper :size="44" :border-radius="12" :style="{
                    backgroundColor: currentHorse?.color || '#18a058',
